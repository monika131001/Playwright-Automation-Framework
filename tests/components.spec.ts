...
test.describe("[COMPONENTS]", () => {
   ...
   */
  test(
    "[Header] Static Messages. Validate that User is able to see messages in Header component.",
    ...
    async ({ page }) => {
      ...
      verificationUtils.elementHasCSSPropertyAndHasValue(
        components.header_logo_swag_labs,
        "Header: Swag Labs",
        "font-size",
        "24px",
      );
   ...
   */
  test(
    "[Header] Navigate to Cart Page. Validate that User is able to navigate to Cart Page using Cart icon.",
    { tag: [tagUtils.REGRESSION] },
    async ({ page }) => {
      // Click on Cart icon
      const components = new Components(page);
   ...
   */
  test(
    "[Footer] Static Messages. Validate that User is able to see messages in Footer component.",
    { tag: [tagUtils.REGRESSION, tagUtils.SANITY] },
    async ({ page }) => {
      const components = new Components(page);
   ...
   */
  test(
    "[Footer] Links navigation. Validate that User is able to navigate to social platforms using icons.",
    { tag: [tagUtils.REGRESSION] },
    async ({ page }) => {
      const components = new Components(page);
      ...
      verificationUtils.elementHasAttributeAndHasValue(
        components.footer_link_twitter,
        "Footer: Twitter link",
        "href",
      ...
      verificationUtils.elementHasAttributeAndHasValue(
        components.footer_link_facebook,
   ...
   */
  test(
    "[Side-Panel] Static Messages. Validate that User is able to see messages in Side-Panel component.",
    { tag: [tagUtils.REGRESSION, tagUtils.SANITY] },
    async ({ page }) => {
      // Open Side-Panel
      const components = new Components(page);
   ...
   */
  test(
    "[Side-Panel] Panel Expand/Collapse. Validate that User is able to expand/collapse panel using icons.",
    { tag: [tagUtils.REGRESSION] },
    async ({ page }) => {
      // Open Side-Panel
      const components = new Components(page);
   ...
   */
  test(
    "[Side-Panel] Link: About. Validate that User is able to navigate to official website using About link.",
    { tag: [tagUtils.REGRESSION] },
    async ({ page }) => {
      // Open Side-Panel
      const components = new Components(page);
  ...
  test("[Side-Panel] Reset App State clears cart items.", { tag: [tagUtils.REGRESSION] }, async ({ page }) => {
    const components = new Components(page);
    const cartPage = new CartPage(page);
    const productsPage = new ProductsPage(page);

    // Step 1: Add product to cart
    await productsPage.product_add_to_cart.first().click();

    // Verify item is added (cart badge should show 1)
    await expect(cartPage.shopping_cart_badge).toHaveText("1");

    // Step 2: Reset app state
    await components.click_side_panel_icon_expand();
    await components.click_side_panel_link_reset_app_state();
